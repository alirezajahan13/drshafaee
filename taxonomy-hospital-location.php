...
<main id="primary" class="site-main mainView">
		<div class="mainIntroSectionParent">
			<?php
			$taxonomy_name = 'hospital-location';

			// Get the current term being viewed
			$current_term = get_queried_object();

			// Check if it's a term in the correct taxonomy
			if ($current_term instanceof WP_Term && $current_term->taxonomy === $taxonomy_name) {
				$display_name = $current_term->name;
			} else {
				$display_name = 'وجود ندارد.';
			}
			?>

			<h1>بیمارستان های محدوده : <?php echo esc_html($display_name); ?></h1>
